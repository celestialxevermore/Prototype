...
def main():
    ...
    FEW_SHOT_EPOCHS = 30 if total_shot <= 16 else 60

    lr_scale = 1 if total_shot <= 16 else 1.0
    ...
    for r in range(R):
        ...
        warmup_epochs_few = max(1, int(args.warmup_ratio * FEW_SHOT_EPOCHS))
        ...
        logger.info(f"[Few-shot][Ep {r+1}/{R}] LR schedule: warmup={warmup_epochs_few}, total={FEW_SHOT_EPOCHS}")

        # K-shot 샘플링 (val 없음)
        train_loader_epi = get_few_shot_embedding_samples(train_loader_t, args)

        # =============================================
        # [변경] Fixed epoch training — no val, no early stopping
        # =============================================
        model_few.train()
        for epoch in range(FEW_SHOT_EPOCHS):
            _ = train_fn_few(model_few, train_loader_epi, crit_t, optimizer_few, device)
            if scheduler_few is not None:
                scheduler_few.step()

        # [변경] Threshold를 train set에서 결정
        model_few.eval()
