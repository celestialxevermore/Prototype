'''
    ...
    args.ours
    기본 형태는 Ours 형식을 따른다.
    edge_attribute에는 feature name 대신 feature description이 들어감. 
    categorical node embedding :  'Sex can have these values: M,F. Current value is M'의 LLM embedding
    numerical node embedding : Transformer(num) * LLM(numerical_name_embedding)
    edge_attribute : feature description ('Sex : sex of the patient')
    여기에서는 --use_edge_attr가 필수가 아님.
'''
...
class Table2EmbeddingTransformer(BaseEstimator, TransformerMixin):
    ...
    def _transform_cat(self, X_categorical, X_categorical_total, cat_name_to_description): 
        ...
        for feature_name in X_categorical.columns:
            ...
            # OURS 고유의 임베딩 구성 방식 유지
            name_value_text = (
                #f"{feature_name} can have these values: {','.join(unique_values)}. "
                #f"Current value is {feature_values_}"
                f"{feature_name} is {feature_values_}"
            )

            if self.llm_model_name == 'sentence-bert':
                with torch.no_grad():
                    # Name embedding
                    feature_name_value_emb = self.llm_model.encode(name_value_text, convert_to_tensor=True)
                    # Description embedding
                    feature_desc_emb = self.llm_model.encode(descriptions_, convert_to_tensor=True)
                    # value embedding 
                    cat_name_value_embeddings.append(feature_name_value_emb)
                    cat_desc_embeddings.append(feature_desc_emb)
                    
            else:
                # Name embedding
                feature_name_value_input = self.tokenizer(name_value_text, return_tensors="pt", padding=True, truncation=True)
                # Description embedding
                feature_desc_input = self.tokenizer(descriptions_, return_tensors="pt", padding=True, truncation=True)
                # Value embedding 
                with torch.no_grad():
                    feature_name_value_output = self.llm_model(**feature_name_value_input)
    ...
    def _get_embeddings(self, X_categorical, X_numerical, cat_name_to_description, num_name_to_description, y, idx):
        ...
        if len(X_categorical.columns) > 0:
            ...
            if num_cat != 0:
                ...
                '''
                    args.name_value
                    node embedding :  "feature name (Gender) is feature value (M), 
                    edge_attribute : feature description (Sex of the patient)
                    여기에서는 Main을 그대로 넣어도 괜찮다. 
                    여기에서는 --use_desc_attr를 선택 사항으로 적용할 수 있음.
                '''
                data.update({
