"""Ideal radar mock v7 — fix: Cardio vs Heart B5 must look CLEARLY different.

v6 problem: both Cardio B5 and Heart B5 had a dominant inward spike at maxHR/thalach,
making them look similar despite different secondary values.

v7 fix:
- Cardio B5: dominant OUTWARD spike on oldpeak (1.4); mild inward notch on maxHR (-0.5).
  Shape = wide pentagon with one big outward bump.
- Heart B5: dominant INWARD spike on maxHR (-1.4); outward peak on cholesterol (1.0).
  Shape = asymmetric polygon with one outward bump AND one inward notch.
- Statlog B5 left as small contained (panel itself looks distinct).
"""
import numpy as np
...
# ============================================================
# v7 — Cardio vs Heart B5 visually distinct shapes.
# ============================================================
sources = [
    ...
    # Cardiovascular  —  B5 dominant OUTWARD spike on oldpeak, mild maxHR notch
    ('Cardiovascular', 'Cardiovascular_Disease_Dataset',
     ...
     [
        ...
        # B5: BIG OUTWARD spike on oldpeak. Mild inward notch on maxHR (-0.5).
        # Shape: pentagon with one big outward bump.
        {'k': 5, 'kind': 'high', 'label_extra': 'WPR=0.70', 'n_eff': 78.4,
         'values': [0.5, 0.6, 0.5, -0.5, 1.4, 0.4]},
    ...
    # Heart  —  B5 OUTWARD chol peak + DEEP INWARD maxHR drop (asymmetric, distinct from Cardio B5)
    ('heart', 'heart',
     ...
     [
        ...
        # B5: chol secondary peak (1.0) + deep maxHR drop (-1.4) + low elsewhere
        # Shape: asymmetric — one outward bump on chol, one deep inward notch on maxHR
        {'k': 5, 'kind': 'high', 'label_extra': 'WPR=0.68', 'n_eff': 64.8,
         'values': [0.4, 0.4, 1.0, 0.5, -1.4, 0.4]},
...
render('/home/eungyeop/LLM/tabular/ProtoLLM_entropic20251217/figures/ideal_radar_mock_4panels_7_with.png',
       show_neff=True)
render('/home/eungyeop/LLM/tabular/ProtoLLM_entropic20251217/figures/ideal_radar_mock_4panels_7_no.png',
       show_neff=False)
