...
def train_and_validate_with_vis(model, train_loader, val_loader, criterion, optimizer, device, epochs, is_binary, patience=10):
    """
    학습 및 검증을 수행하면서 GMM 시각화를 제공하는 함수
    """
    train_losses = []
    val_losses = []
    train_aucs, val_aucs = [], []
    ...
    for epoch in range(epochs):
        ...
        if is_binary:
            ...
            train_precision = precision_score(y_true_train, y_pred_train_bin)
            val_precision = precision_score(y_true_val, y_pred_val_bin)
            
            train_recall = recall_score(y_true_train, y_pred_train_bin)
            val_recall = recall_score(y_true_val, y_pred_val_bin)
            
            train_f1 = f1_score(y_true_train, y_pred_train_bin)
            val_f1 = f1_score(y_true_val, y_pred_val_bin)
            
            train_acc = accuracy_score(y_true_train, y_pred_train_bin)
            val_acc = accuracy_score(y_true_val, y_pred_val_bin)
        else:
            # Multi-class Classification
            train_auc = roc_auc_score(label_binarize(y_true_train, classes=np.unique(y_true_train)), 
                                    label_binarize(y_pred_train, classes=np.unique(y_true_train)), 
                                    multi_class='ovr')
            val_auc = roc_auc_score(label_binarize(y_true_val, classes=np.unique(y_true_val)), 
                                    label_binarize(y_pred_val, classes=np.unique(y_true_val)), 
                                    multi_class='ovr')
            current_threshold = 0  # multi-class에선 필요 없음
        ...
        # 에폭이 끝날 때마다 GMM 시각화 수행 (few-shot 모드일 때만)
        if (epoch % 5 == 0 or epoch == epochs - 1):  # 5 에폭마다 및 마지막 에폭에 시각화
            try:
                vis_batch = next(iter(train_loader))
                
                model.eval()
                with torch.no_grad():
                    # TabularFLM 모델 임베딩 추출
                    if isinstance(vis_batch, dict) and all(k in vis_batch for k in ['cat_name_value_embeddings', 'cat_desc_embeddings']):
                        cat_name_value_embeddings = vis_batch['cat_name_value_embeddings'].to(device).squeeze(-2)
                        cat_desc_embeddings = vis_batch['cat_desc_embeddings'].to(device).squeeze(-2)
                        
                        name_value_embeddings = [cat_name_value_embeddings]
                        desc_embeddings = [cat_desc_embeddings]
                        
                        if 'num_name_value_embeddings' in vis_batch:
                            num_name_value_embeddings = vis_batch['num_name_value_embeddings'].to(device).squeeze(-2)
                            num_desc_embeddings = vis_batch['num_desc_embeddings'].to(device).squeeze(-2)
                            name_value_embeddings.append(num_name_value_embeddings)
                        ...
                        for layer_idx, layer in enumerate(model.layers):
                            norm_x = model.layer_norms[layer_idx](x)
                            attn_output, _ = layer(desc_embeddings, norm_x)
                            x = x + attn_output
                        
                        cls_embeddings = x[:, 0, :]
                        
                        # 여기가 중요! GMM/GMM2 확인 및 시각화
                        if hasattr(model, 'gmm'):
                            # GMM 직접 시각화 (forward 호출 X)
                            from utils.visualization import visualize_gmm_clusters
                            visualize_gmm_clusters(
                                model.gmm, 
                                cls_embeddings,
                                output_dir="visualizations/gmm_clusters_fewshot",
                                step=epoch+1,
                                filename=f"gmm_fewshot_e{epoch+1}.png"
                            )
                            logger.info(f"Few-shot GMM 클러스터 시각화 완료 (에폭 {epoch+1})")
                        elif hasattr(model, 'gmm2'):
                            # GMM2 직접 시각화 (forward 호출 X)
                            from utils.visualization import visualize_gmm_clusters
                            visualize_gmm_clusters(
                                model.gmm2, 
                                cls_embeddings,
                                output_dir="visualizations/gmm_clusters_fewshot",
                                step=epoch+1,
                                filename=f"gmm2_fewshot_e{epoch+1}.png"
                            )
                            logger.info(f"Few-shot GMM2 클러스터 시각화 완료 (에폭 {epoch+1})")
...
def main():
    ...
    # 전체 데이터셋 학습 경로 설정
    full_model_save_path = os.path.join(args.base_dir, 'pretrained_models', args.source_dataset_name, 'full_model.pt')
    os.makedirs(os.path.dirname(full_model_save_path), exist_ok=True)

    if args.few_shot > 0:
        # Full 모델 학습 또는 로드
        if os.path.exists(full_model_save_path):
            logger.info(f"[Source-Only: Full] 기존 학습된 모델 로드: {full_model_save_path}")
            checkpoint = torch.load(full_model_save_path)
            try:
                # 먼저 'model_state_dict' 키로 시도
                model_state_dict = checkpoint['model_state_dict']
            except KeyError:
                # 키가 없으면 체크포인트 자체가 state_dict라고 가정
                model_state_dict = checkpoint
            
            # running_prototypes 형태 수정 (필요한 경우)
            if 'gmm.running_prototypes' in model_state_dict:
                if model_state_dict['gmm.running_prototypes'].dim() == 3:  # [1, 16, 768] 형태인 경우
                    model_state_dict['gmm.running_prototypes'] = model_state_dict['gmm.running_prototypes'].squeeze(0)

            # 수정된 state_dict 로드
            model_full.load_state_dict(model_state_dict)
        else:
            logger.info(f"[Source-Only: Full] 전체 데이터셋으로 모델 학습 시작...")
            ...
            torch.save({
                ...
                'best_val_auc': best_val_auc_full,
                'best_threshold': best_threshold_full
            }, full_model_save_path)
            logger.info(f"Full 모델 저장 완료: {full_model_save_path}")
        ...
        test_acc_full, all_y_true_full, all_y_pred_full) = final_test_evaluate(model_full, test_loader_full_s, criterion, device, is_binary, 
                                                                threshold=0.5)  # best_threshold를 모르는 경우 기본값 사용

        # Few-shot 모델 초기화 - Full 모델의 가중치로 초기화
        logger.info("[Few-shot] Full 모델의 가중치를 활용한 Few-shot 학습 시작...")
        ...
        # 수정된 state_dict 로드
        model_few.load_state_dict(model_state_dict)
        
        # Few-shot 모드 설정 (GMM/GMM2 모듈)
        if hasattr(model_few, 'gmm') and args.use_gmm:
            logger.info("GMM 모듈을 few-shot 모드로 설정")
            model_few.gmm.set_few_shot_mode(True)
        elif hasattr(model_few, 'gmm2') and args.use_gmm2:
            logger.info("GMM2 모듈을 few-shot 모드로 설정")
            model_few.gmm2.set_few_shot_mode(True)

        # GMM 프로토타입 고정 옵션
        if hasattr(model_few, 'gmm') and args.use_gmm:
            logger.info("GMM 프로토타입 레이어 고정 (낮은 learning rate 적용)")
            # GMM 프로토타입을 고정하거나 낮은 learning rate 적용
            gmm_params = []
            non_gmm_params = []
            
            for name, param in model_few.named_parameters():
                if 'gmm' in name:
                    gmm_params.append(param)
                else:
                    non_gmm_params.append(param)
            
            # 다른 파라미터는 일반 learning rate, GMM은 더 낮은 learning rate 적용
            optimizer_few = optim.Adam([
                {'params': non_gmm_params},
                {'params': gmm_params, 'lr': args.source_lr_few}  # GMM에 더 낮은 learning rate
        ...
        # 기존 train_and_validate 함수 대신 새 함수 사용
        ...
        ) = train_and_validate_with_vis(model_few, train_loader_few_s, val_loader_few_s, criterion, optimizer_few, 
                            device, args.train_epochs, is_binary)
        ...
        full_ours_results = wrap_up_results_(
            train_losses=train_losses_full if 'train_losses_full' in locals() else [], 
            val_losses=val_losses_full if 'val_losses_full' in locals() else [],
            test_losses=[], 
            train_aucs=train_aucs_full if 'train_aucs_full' in locals() else [],
            val_aucs=val_aucs_full if 'val_aucs_full' in locals() else [],
            test_aucs=[test_auc_full], 
            train_precisions=train_precisions_full if 'train_precisions_full' in locals() else [],
            val_precisions=val_precisions_full if 'val_precisions_full' in locals() else [],
            test_precisions=[test_precision_full],
            train_recalls=train_recalls_full if 'train_recalls_full' in locals() else [],
            val_recalls=val_recalls_full if 'val_recalls_full' in locals() else [],
            test_recalls=[test_recall_full],
            train_f1s=train_f1s_full if 'train_f1s_full' in locals() else [],
            val_f1s=val_f1s_full if 'val_f1s_full' in locals() else [],
            test_f1s=[test_f1_full],
            all_y_true=[all_y_true_full],
            all_y_pred=[all_y_pred_full],
            best_epoch=best_epoch_full if 'best_epoch_full' in locals() else 0,
            ...
            train_accs=train_accs_full if 'train_accs_full' in locals() else [],
            val_accs=val_accs_full if 'val_accs_full' in locals() else [],
            test_accs=[test_acc_full]
        )
    else:
        # Few-shot 학습을 수행하지 않는 경우 (원래 코드) 
        logger.info(f"[Source-Only: Full] Start Training..")
        # ... 원래 로직 ...
        full_ours_results = None
        few_ours_results = None
