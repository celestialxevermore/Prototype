                ...
                for edge_type in $edge_types; do
                    for num_basis in $num_basis_layers; do
                        for rel_scorer in $relation_scorer_types; do
                            base_dir="test20250902_new_gat_v1_embed_type_${embed_type}_num_basis_${num_basis}_mask_share_across_layers:False_scorer_${rel_scorer}_rel_symmetric:True_no_self_loop:True"
