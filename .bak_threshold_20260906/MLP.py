...
def mlp_benchmark(
    ...
    # 클래스 불균형 보정 — LogReg/RF 의 class_weight='balanced',
    # XGB 의 scale_pos_weight, CatBoost 의 auto_class_weights 에 대응.
    # 보정이 없으면 극단적 불균형에서 전부 0 으로 수렴해 F1 이 0 이 된다.
    if is_binary:
        pw = None
        if getattr(args, 'balance', 'none') == 'balanced':
            n_pos = int(np.sum(np.asarray(y_train) == 1))
            n_neg = int(len(y_train) - n_pos)
            if n_pos > 0:
                pw = torch.tensor([n_neg / n_pos], dtype=torch.float32, device=device)
                print(f"[MLP] balance=balanced -> pos_weight={pw.item():.3f}")
