...
os.environ.setdefault("CUDA_VISIBLE_DEVICES", "4")  # 외부에서 지정하면 그것을 따른다 (기본 4)
...
def get_args():
    ...
    parser.add_argument('--source_data', type=str, default='heart', 
                        choices=['adult','bank','blood','car','communities','credit-g',
                                ...
                                'breast_c1_dfs12', 'breast_c2_dfs12', 'breast_c3_dfs12', 'breast_c4_dfs12',
                                'breast_c1_dfs24', 'breast_c2_dfs24', 'breast_c3_dfs24', 'breast_c4_dfs24'])
    ...
    parser.add_argument('--balance', type=str, default='none', choices=['none','balanced'],
                        help="클래스 불균형 보정. 'balanced'면 XGB/CatBoost 에 pos/neg 가중치를 준다."
                             " LogReg/RF 는 이미 class_weight='balanced' 고정이라 이 값과 무관하다.")
    parser.add_argument('--hp_metric', type=str, default='auc', choices=['auc','auprc'],
                        help='HP 탐색 CV 스코어. 극단적 불균형에서는 auprc 가 더 분별력이 있다.')
