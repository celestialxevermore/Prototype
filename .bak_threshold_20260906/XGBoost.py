...
def xgboost_benchmark(
    ...
    if getattr(args, 'hp_metric', 'auc') == 'auprc':
        scoring = 'average_precision' if is_binary else 'roc_auc_ovr_weighted'
    else:
        scoring = 'roc_auc' if is_binary else 'roc_auc_ovr_weighted'

    # 클래스 불균형 보정 — LogReg/RF 는 class_weight='balanced' 를 이미 쓴다.
    # XGB 의 대응물이 scale_pos_weight = n_neg / n_pos (이진에만 정의됨).
    spw = None
    if is_binary and getattr(args, 'balance', 'none') == 'balanced':
        n_pos = int(np.sum(np.asarray(y_train) == 1))
        n_neg = int(len(y_train) - n_pos)
        spw = (n_neg / n_pos) if n_pos > 0 else None
        print(f"[XGB] balance=balanced → scale_pos_weight={spw}")
        ...
        for max_depth in max_depth_list:
            for n_est in n_estimators_list:
                clf = XGBClassifier(
                    ...
                    **({'scale_pos_weight': spw} if spw is not None else {}),
                )
                try:
                ...
                print(f"[XGB] depth={max_depth}, n_est={n_est} → CV {scoring}={score:.4f}")
                if score > best_score:
                    best_score  = score
    ...
    print(f"[XGB] Best HP: depth={best_max_depth}, n_est={best_n_est} (CV {scoring}={best_score:.4f})")
