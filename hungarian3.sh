#!/bin/bash
gpu_id=4
#export CUDA_VISIBLE_DEVICES=$gpu_id
random_seeds="42"
few_shots="4 8 16 32 64"
num_basis_layers="2"
num_basis_heads="1"
num_graphs="8 10"
echo "=== 시나리오 1 VQVAE: 'Heart_disease_statlog', 'Cardiovascular_Disease_Dataset', 'heart_target_3', 'heart_target_4' -> hungarian ==="
for random_seed in $random_seeds; do
    for n_graphs in $num_graphs; do
        for few_shot in $few_shots; do
            base_dir="test20251106_MultiSource_VQVAE_Target_hungarian"
            echo "Running experiment - seed:${random_seed}, few_shot:${few_shot}"
            CUDA_VISIBLE_DEVICES=$gpu_id OMP_NUM_THREADS=10 python main_S.py \
                --random_seed $random_seed \
                --source_data Heart_disease_statlog Cardiovascular_Disease_Dataset heart_target_3 heart_target_4 \
                --target_data hungarian \
                --base_dir $base_dir \
                --few_shot $few_shot \
                --n_graphs $n_graphs \
                --num_basis_heads $num_basis_heads \
                --num_basis_layers $num_basis_layers \
                --additional_FGW \
                --des MultiSource_VQVAE_Additional_FGW_20251106_hungarian
        done
    done
done

echo "=== 모든 실험 완료 ==="