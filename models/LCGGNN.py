...
class lightGraphNeuralNet(nn.Module):
    ...
    def forward(self, Fy_res : torch.Tensor, Ay_sel : torch.Tensor):
        ...
        # Message Passing 
        agg = torch.bmm(1.0 - Ay_sel, Fy_res)
...
#     """
#     Graph-level readout : Mean Pooling + Projoection
#     """
...
class GraphReadout(nn.Module):
    ...
    def forward(self, node_embeddings: torch.Tensor):
        ...
        Returns:
            graph_embedding : [B, D]
        """
        attn_scores = self.attn(node_embeddings)        # [B, K, 1]
        attn_weights = torch.softmax(attn_scores, dim=1) # [B, K, 1]
        graph_embedding = (attn_weights * node_embeddings).sum(dim=1)  # [B, D]
        return self.norm(self.proj(graph_embedding))
...
class LatentCompositeGNN(nn.Module):
    def __init__(self, args, input_dim: int, hidden_dim: int, dropout: float = 0.1):
        super().__init__()
        self.args = args
        self.n_graphs = args.n_graphs
        self.input_dim = input_dim
        self.num_layers = 2

        self.graph_gnns = nn.ModuleList([
            nn.ModuleList([
                lightGraphNeuralNet(self.input_dim, hidden_dim, dropout)
        ...
        self.readouts = nn.ModuleList([
            GraphReadout(self.input_dim, hidden_dim, self.input_dim, dropout)
            for _ in range(self.n_graphs)
        ])
    ...
    def forward(self, Fy_res: torch.Tensor, Ay_sel: torch.Tensor):
        ...
        for m in range(self.n_graphs):
            ...
            for layer in self.graph_gnns[m]:
                node_out = layer(node_out, adj_in)
