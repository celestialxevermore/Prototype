...
class Prototype:
    ...
    def compute_current_task_prototypes(self, embeddings, labels):
        """
        현재 task의 support set으로부터 prototype 생성
        Same dataset이므로 task_classes 필요 없음
        
        Args:
            embeddings: support set embeddings
            labels: support set labels (0, 1, 2, ...)
        
        Returns:
            current_prototypes: [num_classes, input_dim]
        """
        current_prototypes = []
        ...
        for class_id in range(self.num_classes):
            ...
            if len(class_embeddings) > 0:
                ...
                current_prototypes.append(class_prototype)
            else:
                # 해당 클래스가 batch에 없으면 zero vector
                current_prototypes.append(torch.zeros(self.input_dim, device=self.device))
        
        return torch.stack(current_prototypes)
    
    def compute_prototype_alignment_loss(self, embeddings, labels):
        """
        Support set prototype과 source prototype 간의 alignment loss
        Same dataset이므로 간단함
        """
        if self.source_prototypes is None:
            return torch.tensor(0.0, device=self.device)
        
        # Current task prototypes 생성
        current_prototypes = self.compute_current_task_prototypes(embeddings, labels)
        
        # Alignment loss (L2 distance)
        alignment_loss = F.mse_loss(current_prototypes, self.source_prototypes)
        #pdb.set_trace()
    ...
    def predict_with_prototype_guidance(self, embeddings, model_logits):
        if self.source_prototypes is None:
            return model_logits
        
        # Prototype 거리 기반 logits
        prototype_distances = torch.cdist(embeddings, self.source_prototypes)
        prototype_logits = -prototype_distances
        
        # Binary case 처리
        if model_logits.shape[1] == 1:
            # Prototype logits에서 positive class만 선택 (class 1)
            prototype_logits = prototype_logits[:, 1:2]
        
        # Logit space에서 직접 결합
        alpha = 0.5
        beta = 0.5
        final_logits = alpha * model_logits + beta * prototype_logits
...
class PrototypeModel(nn.Module):
    ...
    def forward(self, batch, y):
        ...
        if self.training_phase == "Full":
            ...
            # Phase 2: Few-shot training with prototype alignment
            if self.Prototype.source_prototypes is not None:
                # Support set prototype과 source prototype alignment
                alignment_loss = self.Prototype.compute_prototype_alignment_loss(
                    cls_embeddings, labels_for_prototype
                )
                total_loss = classification_loss + 0.2 * alignment_loss  # 약한 regularization
                #total_loss = classification_loss
                return total_loss
            else:
                logger.warning("Source prototypes not loaded. Using classification loss only.")
    ...
    def predict(self, batch):
        """
        Test time prediction with prototype guidance
        """
        cls_embedding = self.base_model.compute_cls_embedding(batch)
        model_logits = self.base_model.predictor(cls_embedding)
        
        # Test time에는 prototype guidance 적용
        if self.Prototype.source_prototypes is not None:
            final_logits = self.Prototype.predict_with_prototype_guidance(
                cls_embedding, model_logits
            )
            return final_logits
        else:
            return model_logits
