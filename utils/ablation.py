...
def determine_del_feat_by_scenario(scenario_id, important_features, all_features, unimportant_features):
  ...
  if scenario_id == 1:
      ...
      # "가장 안중요한 3개만 유지" = 나머지 모두 제거
      unimportant_set = set(unimportant_features[:3])  # 가장 안중요한 3개만 선택
      return [f for f in all_features if f not in unimportant_set]
  else:
      return []
