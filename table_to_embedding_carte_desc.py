'''
    ...
    하지만 edge_attribute에는 feature name 대신 feature description이 들어감. 
    ...
    따라서, table_to_embedding_carte, table_to_embedding_carte_desc에서는 반드시 --use_desc_attr를 써야 한다.
    왜냐하면 node에는 그냥 'M' 밖에 없기 때문에 feature 정보를 주지 않으면 그냥 쓰레기값으로 모델이 인지하기 때문. 
'''
        ...
        if len(X_numerical.columns) > 0: 
            ...
            if num_num > 0:
                ...
                data.update({
                    ...
                    'num_desc_embeddings': num_desc_embeddings,     # description embedding (edge attribute용)
