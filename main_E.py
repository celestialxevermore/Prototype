...
def main():
    ...
    # ── 결과 저장 디렉토리 ──
    exp_results_dir = f"/storage/personal/eungyeop/experiments/experiments/source_to_source_{args.base_dir}/{args.exp_mode}"
    ...
    # ==================================================================
        ...
        for src_name in src_list:
            logger.info(f"\n--- [Case 1 Fine-tune] {src_name} (alpha={alpha}) ---")
        ...
        result_file = os.path.join(
            exp_results_dir,
            f"case1_alpha{alpha:.2f}_seed{args.random_seed}_{args.run_tag}.json"
        )
        save_data = {
            'exp_mode': 'case1',
            'sources': src_list,
    ...
    # ==================================================================
        ...
        # ── Fine-tune: 각 source별로, alpha=1.0 (해당 source 전체 사용) ──
        ft_sources = []
        ft_auc, ft_auprc, ft_acc, ft_f1 = [], [], [], []
        ft_precision, ft_recall, ft_loss = [], [], []

        for src_name in src_list:
            logger.info(f"\n--- [Case 2 Fine-tune] {src_name} (alpha=1.0) ---")
            model.load_state_dict({k: v.to(device) for k, v in pretrained_state.items()})
            fix_seed(args.random_seed)
        ...
        result_file = os.path.join(
            exp_results_dir,
            f"case2_alpha{alpha:.2f}_seed{args.random_seed}_{args.run_tag}.json"
        )
        save_data = {
            'exp_mode': 'case2',
            'sources': src_list,
