...
def objective(trial, base_args):
    """Optuna objective function - Full Shot 최적화"""
    # 하이퍼파라미터 제안
    dropout_rate = trial.suggest_float('dropout', 0.1, 0.5, step=0.1)
    n_heads = trial.suggest_categorical('heads', [2, 4, 8])
    num_layers = trial.suggest_categorical('layers', [2, 3, 4])
    
    # ✨ Full shot을 위한 learning rate (보통 few-shot보다 높음)
    source_lr_full = trial.suggest_float('lr_full', 1e-5, 1e-3, log=True)
    attn_type = trial.suggest_categorical('attn_type', ['gat_v1', 'gat_v2'])
    ...
    for seed in seeds:
        try:
            ...
            # ✨ Full Shot 모델 생성 및 학습
            model_full = Model(args, args.input_dim, args.hidden_dim, args.output_dim, 
                             args.num_layers, args.dropout_rate, args.llm_model, 
                             experiment_id, mode="Full")
            
            model_full = model_full.to(device)
            
            # ✨ Full shot learning rate와 weight decay 적용
            optimizer_full = optim.Adam(model_full.parameters(), lr=source_lr_full, 
                                      weight_decay=weight_decay)
            
            # ✨ 전체 데이터로 학습 및 평가
            val_auc = train_and_validate(args, model_full, train_loader_full_s, val_loader_full_s, 
                                       criterion, optimizer_full, device, args.train_epochs, 
                                       is_binary, mode="Full", trial=trial)
            val_aucs.append(val_auc)
    ...
    # NaN/Inf 체크
    if np.isnan(mean_val_auc) or np.isinf(mean_val_auc):
        return 0.0
    
    logger.info(f"Trial {trial.number}: Full AUC={mean_val_auc:.4f} (lr={source_lr_full:.2e}, weight_decay={weight_decay:.2e})")
    return mean_val_auc
...
def main():
    ...
    with open(results_path, 'w') as f:
        ...
        for i, trial in enumerate(study.trials_dataframe().nlargest(10, 'value').iterrows()):
            f.write(f"{i+1}. Trial {trial[1]['number']}: {trial[1]['value']:.4f} - "
                    f"dropout={trial[1]['params_dropout']}, "
                    f"heads={trial[1]['params_heads']}, "
                    f"layers={trial[1]['params_layers']}, "
                    f"lr={trial[1]['params_lr_full']:.2e}, "
                    f"weight_decay={trial[1]['params_weight_decay']:.2e}\n")
