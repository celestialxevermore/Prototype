...
def pretrain_and_eval_sources(args, model, device, sources, patience=10):
    ...
    if total_epochs > 0:
        ...
        scheduler_ep  = make_warmup_cosine_epochs(
            ...
            warmup_epochs=warmup_epochs,     # ⬅ ratio가 아니라 정수 epoch 수
    ...
    # -----------------------------
    # 소스별 threshold 산출(Val) → train/val/test 지표 계산 (전부 src_idx 고정 로더 사용)
    # -----------------------------
...
def main():
    ...
    if os.path.exists(ckpt_path):
        ...
        logger.info(f"[Pretrain] No checkpoint at {ckpt_path} → run pretraining.")
    ...
    if loaded_pretrain:
        if args.few_shot == 4:
            ...
            logger.info("[Full] Using loaded pretrain (no source report for this few-shot).")

    # 3) few-shot 적응: 가중치 복사 → freeze 정책 적용
    ...
    # 5) Few-shot: support 재샘플링 R회 에피소드 평균
    R = int(getattr(args, 'support_resamples', 1))
    logger.info(f"[Few-shot] support resamples R = {R}")

    # 에피소드마다 pretrain 가중치로 초기화할 수 있게 base state를 CPU에 보관
    ...
    for r in range(R):
        ...
        # ✅ epoch-wise warmup + cosine scheduler (few-shot용)
