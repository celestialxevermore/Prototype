"""
Attention Maps Inference 스크립트 (TabularFLM_S 지원)

학습 완료된 TabularFLM_S 모델을 로드하여 전체 데이터셋에 대해 attention maps를 추출하고
Multi-source K-means 클러스터링을 수행합니다.
...
class AttentionInference:
    def __init__(self, checkpoint_dir, device='cuda', auto_del_feat=None):
        ...
        # 체크포인트 키 이름 통일 - 다양한 키 이름 지원
        epoch = self.checkpoint.get('epoch', 'unknown')
        
        # val_auc 키 이름이 다양할 수 있음
        val_auc = None
        if 'val_auc' in self.checkpoint:
            val_auc = self.checkpoint['val_auc']
        elif 'val_auc_mean' in self.checkpoint:
            val_auc = self.checkpoint['val_auc_mean']
        elif 'val_aucs' in self.checkpoint:
            # val_aucs가 리스트인 경우 평균
            val_aucs = self.checkpoint['val_aucs']
            if isinstance(val_aucs, list):
                val_auc = sum(val_aucs) / len(val_aucs)
            else:
                val_auc = val_aucs
        
        if val_auc is not None:
            logger.info(f"Checkpoint epoch: {epoch}, Val AUC: {val_auc:.4f}")
        else:
            logger.info(f"Checkpoint epoch: {epoch}, Val AUC: not available")
    ...
    def _load_model(self):
        ...
        logger.info("TabularFLM_S Model loaded and set to evaluation mode")
    
    def _prepare_dataloaders(self):
        """데이터로더 준비"""
        fix_seed(self.args.random_seed)
        
        # source_data가 리스트인 경우 처리
        if isinstance(self.args.source_data, (list, tuple)):
            # 여러 소스 데이터셋의 데이터로더들을 준비
            self.source_loaders = {}  # 소스별로 분리된 데이터로더
            
            for source_name in self.args.source_data:
                try:
                    results = prepare_embedding_dataloaders(self.args, source_name)
                    train_loader, val_loader, test_loader = results['loaders']
                    ...
                    self.source_loaders[source_name] = combined_loader
                    logger.info(f"Loaded dataloader for {source_name}")
                    
                except Exception as e:
                    logger.warning(f"Failed to load {source_name}: {e}")
                    continue
            
            if not self.source_loaders:
                raise ValueError("No source dataloaders could be loaded")
            
            # 첫 번째 소스의 정보를 기본값으로 사용
            first_source = list(self.source_loaders.keys())[0]
            self.train_loader = self.source_loaders[first_source]
            self.num_classes = results['num_classes']
            
            logger.info(f"Prepared {len(self.source_loaders)} source dataloaders individually")
            ...
            # 단일 소스도 source_loaders에 추가
            self.source_loaders = {self.args.source_data: self.combined_loader}
        
        # Target 데이터셋 추가 (가변적 처리)
        target_data = getattr(self.args, 'target_data', 'heart')
        
        # target_data가 리스트인 경우 처리
        if isinstance(target_data, (list, tuple)):
            target_loaders = []
            for target_name in target_data:
                try:
                    target_results = prepare_embedding_dataloaders(self.args, target_name)
                    target_loaders.append(target_results['loaders'])
                    logger.info(f"Loaded target dataloader for {target_name}")
                except Exception as e:
                    logger.warning(f"Failed to load target {target_name}: {e}")
                    continue
            
            if target_loaders:
                # 모든 target 데이터셋을 결합
                target_datasets = []
                for train_loader, val_loader, test_loader in target_loaders:
                ...
                # 기존 combined_loader의 데이터셋과 target 데이터셋을 결합
                if hasattr(self, 'combined_loader'):
                    existing_dataset = self.combined_loader.dataset
                    all_datasets = [existing_dataset] + target_datasets
                    final_combined_dataset = ConcatDataset(all_datasets)
                else:
                    final_combined_dataset = ConcatDataset(target_datasets)
                
                # 최종 결합된 데이터로더 생성
                batch_size = getattr(self, 'train_loader', target_loaders[0][0]).batch_size
                num_workers = getattr(getattr(self, 'train_loader', target_loaders[0][0]), 'num_workers', 0)
                ...
                logger.info(f"Added {len(target_data)} target datasets to combined dataloader")
            
        else:
            # 단일 target 데이터셋
            try:
                target_results = prepare_embedding_dataloaders(self.args, target_data)
                target_train_loader, target_val_loader, target_test_loader = target_results['loaders']
                
                # Target 데이터셋도 결합된 데이터로더에 추가
                target_datasets = [
                    target_train_loader.dataset,
                    target_val_loader.dataset,
                ...
                # 최종 결합된 데이터로더 생성
                batch_size = getattr(self, 'train_loader', target_train_loader).batch_size
                num_workers = getattr(getattr(self, 'train_loader', target_train_loader), 'num_workers', 0)
                ...
                logger.info(f"Added target dataset '{target_data}' to combined dataloader")
                
            except Exception as e:
                logger.warning(f"Failed to load target dataset '{target_data}': {e}")
        ...
        logger.info(f"Dataloaders prepared for dataset: {self.args.source_data} + target: {target_data}")
    
    def extract_attention_maps(self, data_loader):
        """TabularFLM_S 모델에서 attention maps 추출 - 각 소스별로 개별 처리"""
        attention_data = {
            'layer_0': [],
            'layer_1': [], 
            'layer_2': [],
            'basis_layer': [],  # Basis layer 추가
            'coordinates': [],   # Coordinator 좌표 추가
            'labels': [],
            'sample_ids': [],
            'feature_names': None,
            'source_target_info': []  # Source/Target 구분 정보
        }
        
        sample_count = 0
        
        # 각 소스별로 개별적으로 처리
        if hasattr(self, 'source_loaders'):
            # source_loaders가 있으면 각 소스별로 개별 처리
            for source_name, source_loader in self.source_loaders.items():
                logger.info(f"Processing attention maps for source: {source_name}")
                source_data = self._extract_from_single_loader(source_loader, source_name, 'source')
                
                # 데이터 병합
                for key in attention_data.keys():
                    if key != 'feature_names':
                        attention_data[key].extend(source_data[key])
                    elif attention_data['feature_names'] is None:
                        attention_data['feature_names'] = source_data['feature_names']
                
                sample_count += len(source_data['labels'])
                logger.info(f"Added {len(source_data['labels'])} samples from {source_name}")
        
        # Target 데이터도 처리
        if hasattr(self, 'combined_loader'):
            logger.info(f"Processing attention maps for target: {getattr(self.args, 'target_data', 'heart')}")
            
            # Target 데이터만 별도로 로드
            target_data_name = getattr(self.args, 'target_data', 'heart')
            try:
                target_results = prepare_embedding_dataloaders(self.args, target_data_name)
                target_train_loader, target_val_loader, target_test_loader = target_results['loaders']
                
                # Target 데이터셋 결합
                from torch.utils.data import ConcatDataset, DataLoader
                target_combined_dataset = ConcatDataset([
                    target_train_loader.dataset,
                    target_val_loader.dataset,
                    target_test_loader.dataset
                ])
                
                target_combined_loader = DataLoader(
                    target_combined_dataset,
                    batch_size=target_train_loader.batch_size,
                    shuffle=False,
                    num_workers=getattr(target_train_loader, 'num_workers', 0)
                )
                
                target_data = self._extract_from_single_loader(target_combined_loader, target_data_name, 'target')
                
                # Target 데이터 추가
                for key in attention_data.keys():
                    if key != 'feature_names':
                        attention_data[key].extend(target_data[key])
                    elif attention_data['feature_names'] is None:
                        attention_data['feature_names'] = target_data['feature_names']
                
                sample_count += len(target_data['labels'])
                logger.info(f"Added {len(target_data['labels'])} target samples from {target_data_name}")
                
            except Exception as e:
                logger.warning(f"Failed to process target data {target_data_name}: {e}")
        
        logger.info(f"Extracted attention maps for {sample_count} total samples")
        return attention_data
    
    def _extract_from_single_loader(self, data_loader, name, data_type):
        """단일 데이터로더에서 attention maps 추출"""
        attention_data = {
            'layer_0': [],
            'layer_1': [], 
            'layer_2': [],
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(data_loader):
                ...
                # TabularFLM_S 모델의 attention weights 추출
                ...
                for sample_idx in range(batch_size):
                    ...
                    for layer_idx, layer_attention in enumerate(attention_weights[:-1]):  # basis layer 제외
                    ...
                    # Basis layer attention 저장
                    basis_attention = attention_weights[-1][sample_idx]  # [k_basis, seq_len, seq_len]
                    attention_data['basis_layer'].append(basis_attention.detach().cpu().numpy())
                    
                    # Coordinator 좌표 저장
                    sample_coordinates = coordinates[sample_idx].detach().cpu().numpy()
                    attention_data['coordinates'].append(sample_coordinates)
                    
                    # Source/Target 구분 정보 저장
                    attention_data['source_target_info'].append(data_type)
                ...
                if batch_idx % 10 == 0:
                    logger.info(f"Processed {sample_count} samples from {name}...")
        
        logger.info(f"Extracted {sample_count} samples from {name}")
        return attention_data
    
    def _extract_attention_from_model(self, batch):
        """TabularFLM_S 모델에서 attention weights, 예측값, 좌표 추출"""
        ...
        return pred, attention_weights, coordinates

    def visualize_head_attention_heatmaps(self, data_loader, output_dir, max_samples=5):
        """개별 샘플별 Head Attention 히트맵과 Coordinate Bar Graph 시각화 - Source별 개별 처리"""
        output_dir = Path(output_dir)
        output_dir.mkdir(parents=True, exist_ok=True)
        
        # Source와 Target 폴더 생성
        ...
        if isinstance(self.args.source_data, (list, tuple)):
            for source_name in self.args.source_data:
                ...
                source_folders[source_name] = source_folder
        ...
        if isinstance(target_data, (list, tuple)):
            ...
            target_folder = target_dir / target_data
            target_folder.mkdir(exist_ok=True)
        
        # ========== 1. SOURCE 데이터 개별 처리 ==========
        if hasattr(self, 'source_loaders'):
            logger.info("🚀 Processing source datasets individually...")
            ...
            for source_name, source_loader in self.source_loaders.items():
                ...
                with torch.no_grad():
                    for batch_idx, batch in enumerate(source_loader):
                        ...
                        for sample_idx in range(batch_size):
                            ...
                            # 1. Coordinate Bar Graph 생성
                            self._create_coordinate_bargraph(
                                coordinates[sample_idx], 
                                save_dir, 
                                source_sample_count, 
                                'source'
                            )
                            ...
                            self._create_attention_heatmap(
                                attention_weights[-1][sample_idx], 
                                feature_names, 
                                save_dir, 
                                source_sample_count, 
                                'source'
                ...
                logger.info(f"✅ Source {source_name}: {source_sample_count} samples processed")
            
            logger.info(f"🎯 Total source samples processed: {total_source_samples}")
        
        # ========== 2. TARGET 데이터 처리 ==========
        logger.info(f"🎯 Processing target dataset: {target_data}")
        target_sample_count = 0
        
        # Target 데이터만 별도로 로드하여 feature names 추출
        target_data_name = getattr(self.args, 'target_data', 'heart')
        try:
            target_results = prepare_embedding_dataloaders(self.args, target_data_name)
            target_train_loader, target_val_loader, target_test_loader = target_results['loaders']
            ...
            with torch.no_grad():
                for batch_idx, batch in enumerate(target_combined_loader):
                    if target_sample_count >= max_samples:
                        break
            ...
            logger.info(f"✅ Target attention visualization completed: {target_sample_count} samples")
            
        except Exception as e:
            logger.warning(f"❌ Failed to process target data {target_data_name}: {e}")
            logger.warning(f"Target feature names extraction failed: {e}")
        
        # ========== 3. 최종 요약 ==========
        total_samples = (total_source_samples if hasattr(self, 'source_loaders') else 0) + target_sample_count
        logger.info(f"🎉 Head attention visualization completed! Total: {total_samples} samples")
        logger.info(f"   - Source: {total_source_samples if hasattr(self, 'source_loaders') else 0} samples")
        logger.info(f"   - Target: {target_sample_count} samples")

    def _create_coordinate_bargraph(self, coordinates, save_dir, sample_idx, data_type):
        """Coordinate Bar Graph 생성 (참조 이미지와 동일한 형태)"""
        k_basis = coordinates.shape[0]
        
        fig, ax = plt.subplots(1, 1, figsize=(10, 6))
        
        # Head Index와 Weight 준비
        head_indices = list(range(k_basis))
        weights = coordinates.cpu().numpy()
        
        # Bar Graph 생성
        bars = ax.bar(head_indices, weights, color='skyblue', alpha=0.8, edgecolor='navy', linewidth=1)
        
        # 각 막대 위에 정확한 값 표시
        for i, (bar, weight) in enumerate(zip(bars, weights)):
            ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.01, 
                    f'{weight:.3f}', ha='center', va='bottom', fontweight='bold', fontsize=10)
        
        # 축 설정
        ax.set_xlabel('Head Index', fontsize=12, fontweight='bold')
        ax.set_ylabel('Weight', fontsize=12, fontweight='bold')
        ax.set_title(f'{data_type.title()} - Sample {sample_idx} - Head Weights (Coordinates)', 
                    fontsize=14, fontweight='bold', pad=20)
        
        # X축 틱 설정
        ax.set_xticks(head_indices)
        ax.set_xticklabels([f'Head {i}' for i in head_indices], fontsize=10)
        ...
        # 저장
        coord_path = save_dir / f'sample_{sample_idx}_coordinate.png'
        fig.savefig(coord_path, dpi=300, bbox_inches='tight')
        plt.close(fig)
        
        logger.info(f"Coordinate bar graph saved: {coord_path}")

    def _create_attention_heatmap(self, basis_attention, feature_names, save_dir, sample_idx, data_type):
        """Attention Map 히트맵 생성 - 실제 변수명 활용"""
        k_basis = basis_attention.shape[0]  # 동적 head 수
        
        # attention map의 실제 크기 확인
        seq_len = basis_attention.shape[1]  # 실제 sequence length
        ...
        for head_idx in range(k_basis):
            if head_idx < len(axes):
                ax = axes[head_idx]
                attention_map = basis_attention[head_idx].cpu().numpy()
                
                im = ax.imshow(attention_map, cmap='viridis', interpolation='nearest')
                ax.set_title(f'Basis Head {head_idx}', fontsize=12, fontweight='bold')
                
                # 축 라벨 설정 - 실제 변수명 우선 사용
                if seq_len == len(feature_names) + 1:  # CLS + features
                    all_node_names = ["CLS"] + feature_names
                    logger.info(f"✅ Using actual feature names: CLS + {len(feature_names)} features")
                elif seq_len == len(feature_names):  # features만 (CLS 없음)
                    all_node_names = feature_names
                    logger.info(f"✅ Using feature names only: {len(feature_names)} features")
                else:
                    # 크기가 맞지 않으면 정확한 feature 수로 재계산
                    logger.warning(f"⚠️  Size mismatch: seq_len={seq_len}, feature_names={len(feature_names)}")
                    
                    # seq_len이 14라면 CLS + 13개 features
                    if seq_len > 1:
                        actual_feature_count = seq_len - 1  # CLS 제외
                        # feature_names가 부족하면 기본 이름으로 보완
                        if len(feature_names) < actual_feature_count:
                            missing_count = actual_feature_count - len(feature_names)
                            extended_names = list(feature_names) + [f'feature_{i+len(feature_names)}' for i in range(missing_count)]
                            all_node_names = ["CLS"] + extended_names
                            logger.info(f"🔧 Extended feature names: {len(extended_names)} total features")
                        else:
                            all_node_names = ["CLS"] + feature_names[:actual_feature_count]
                            logger.info(f"🔧 Truncated feature names: {actual_feature_count} features")
                    else:
                        all_node_names = [f"node_{i}" for i in range(seq_len)]
                        logger.warning(f"⚠️  Using fallback names for seq_len={seq_len}")
                
                ax.set_xticks(np.arange(seq_len))
                ax.set_yticks(np.arange(seq_len))
                
                # x축과 y축 라벨 모두 표시 (가독성 향상)
                ax.set_xticklabels(all_node_names, rotation=90, fontsize=8)
                ax.set_yticklabels(all_node_names, fontsize=8)
                
                # 각 셀에 값 표시 (head 수가 많으면 생략)
                if k_basis <= 12:  # 12개 이하일 때만 값 표시
                    for i in range(seq_len):
                        for j in range(seq_len):
                            value = attention_map[i, j]
                            threshold = attention_map.max() * 0.5
        ...
        # 사용하지 않는 서브플롯 숨기기
        for i in range(k_basis, len(axes)):
            axes[i].set_visible(False)
        
        plt.suptitle(f'{data_type.title()} - Sample {sample_idx} - Head Attention Analysis ({k_basis} heads, {seq_len}x{seq_len})', 
                    fontsize=16, y=0.98)
        plt.tight_layout()
        
        # 저장
        attention_path = save_dir / f'sample_{sample_idx}_attention.png'
        fig.savefig(attention_path, dpi=300, bbox_inches='tight')
        plt.close(fig)
        
        logger.info(f"Attention heatmap saved: {attention_path}")

    def extract_feature_names(self, batch):
        """배치에서 feature names 추출 - TabularFLM_S 모델의 메서드 활용"""
        try:
            # TabularFLM_S 모델의 extract_feature_names 메서드 직접 호출
            ...
            if feature_names and len(feature_names) > 0:
                ...
                logger.warning("⚠️  Model returned empty feature names")
                
        except Exception as e:
            logger.warning(f"⚠️  Failed to extract feature names from model: {e}")
        
        # Fallback: 기존 로직 사용
        logger.warning("⚠️  Using fallback feature name extraction")
        
        # 1. 배치에서 직접 feature_names 추출
        if 'feature_names' in batch:
            feature_names = batch['feature_names']
            if isinstance(feature_names, (list, tuple)) and len(feature_names) > 0:
                logger.info(f"✅ Extracted {len(feature_names)} feature names from batch: {feature_names[:5]}...")
                return feature_names
        
        # 2. 이미 저장된 feature_names 사용
        if hasattr(self, 'feature_names') and self.feature_names is not None:
            logger.info(f"✅ Using stored feature names: {len(self.feature_names)} features")
            return self.feature_names
        
        # 3. 실제 embedding 차원에서 정확한 feature 수 계산
        total_features = 0
        
        if 'cat_name_value_embeddings' in batch:
            cat_features = batch['cat_name_value_embeddings'].shape[1]
            total_features += cat_features
            logger.info(f"📊 Categorical features: {cat_features}")
        
        if 'num_prompt_embeddings' in batch:
            num_features = batch['num_prompt_embeddings'].shape[1]
            total_features += num_features
            logger.info(f"�� Numerical features: {num_features}")
        
        if total_features > 0:
            # CLS 토큰이 이미 포함되어 있을 수 있으므로 확인
            if total_features > 1:  # CLS + features
                feature_count = total_features - 1  # CLS 제외
            else:
                feature_count = total_features
            logger.info(f"📊 Total estimated features: {feature_count}")
        else:
            # fallback: input_dim 사용
            feature_count = self.args.input_dim - 1  # CLS 제외
            logger.info(f"📊 Using input_dim fallback: {feature_count} features")
        
        # 4. 기본 feature names 생성
        default_names = [f'feature_{i}' for i in range(feature_count)]
        logger.warning(f"⚠️  Using default feature names: {default_names[:5]}...")
    ...
    def save_attention_maps_by_cluster(self, attention_data, clustering_results, output_dir, layer_idx):
        """클러스터별로 attention maps 저장 - basis layer 사용"""
        layer_dir = Path(output_dir) / f'basis_layer'
        
        # 클러스터별 폴더 생성
        unique_clusters = np.unique(clustering_results['cluster_assignments'])
        ...
        for sample_idx, cluster_id in enumerate(clustering_results['cluster_assignments']):
            # basis layer attention map 사용
            attention_map = attention_data['basis_layer'][sample_idx]  # [k_basis, seq_len, seq_len]
        ...
        logger.info(f"Saved basis layer attention maps by cluster")
    ...
    def perform_multi_source_clustering(self, attention_data, layer_idx=2, n_clusters=5, output_dir=None):
        """Multi-source 환경에서 클러스터링 수행"""
        if output_dir:
            output_dir = Path(output_dir)
            output_dir.mkdir(parents=True, exist_ok=True)
        
        # Coordinates만 사용 (attention maps는 크기가 다를 수 있음)
        coordinates = np.stack(attention_data['coordinates'])      # [n_samples, k_basis]
        source_target_info = attention_data['source_target_info']
        labels = np.array(attention_data['labels'])
        sample_ids = np.array(attention_data['sample_ids'])
        feature_names = attention_data['feature_names']
        
        logger.info(f"Performing multi-source clustering on coordinates with {len(coordinates)} samples")
        logger.info(f"Coordinates shape: {coordinates.shape}")
        
        # 좌표 기반 클러스터링 (Multi-source 환경에 적합)
        kmeans = KMeans(n_clusters=n_clusters, random_state=42, n_init=20)
        ...
        if output_dir:
            ...
            # KMeans 모델 저장
            kmeans_path = clustering_dir / f'basis_layer_kmeans_model.pkl'
            with open(kmeans_path, 'wb') as f:
                pickle.dump(kmeans, f)
        
        # Multi-source 클러스터링 시각화
        if output_dir:
            self._visualize_multi_source_clustering(
                coordinates, cluster_assignments, source_target_info, 
                labels, feature_names, output_dir, n_clusters
            )
        ...
        return {
            ...
            'source_target_info': source_target_info,
            'labels': labels,
            'sample_ids': sample_ids,
            'layer_idx': layer_idx
        }

    def _visualize_multi_source_clustering(self, coordinates, cluster_assignments, source_target_info, 
                                         labels, feature_names, output_dir, n_clusters):
        """Multi-source 클러스터링 시각화 (가변 head 수 지원)"""
        # t-SNE 차원 축소
        ...
        # 1. Multi Source Clustering (t-SNE)
        ax1 = axes[0, 0]
        ...
        for i, cluster_id in enumerate(unique_clusters):
            ...
            # Source 데이터 (원형) - 더 투명하게
            source_mask = np.array([info == 'source' for info in source_target_info])
            source_cluster_mask = cluster_mask & source_mask
            if np.any(source_cluster_mask):
                ax1.scatter(tsne_embeddings[source_cluster_mask, 0], 
                           tsne_embeddings[source_cluster_mask, 1],
                           c=[colors[i]], marker='o', s=50, alpha=0.5, zorder=1,
                           label=f'Cluster {cluster_id} (Source)')
            
            # Target 데이터 (삼각형) - 더 불투명하게, 앞쪽에 그리기
            target_mask = np.array([info == 'target' for info in source_target_info])
            target_cluster_mask = cluster_mask & target_mask
            if np.any(target_cluster_mask):
                ax1.scatter(tsne_embeddings[target_cluster_mask, 0], 
                           tsne_embeddings[target_cluster_mask, 1],
                           c=[colors[i]], marker='^', s=60, alpha=0.9, zorder=2,
                           label=f'Cluster {cluster_id} (Target)')
        
        ax1.set_title('Multi Source Clustering (Source: O, Target: Δ)\nMulti Source Coordinate Clustering Analysis', 
        ...
        # 2. Cluster Mean Coordinates (Heatmap) - 가변 head 수 지원
        ax2 = axes[0, 1]
        # 이미 계산된 cluster_centers 사용 (perform_multi_source_clustering에서 전달받음)
        k_basis = coordinates.shape[1]  # 동적 head 수
        
        # 클러스터별 평균 좌표 계산
        cluster_centers = np.zeros((n_clusters, k_basis))
        for i in range(n_clusters):
            cluster_mask = cluster_assignments == i
            if np.any(cluster_mask):
                cluster_centers[i] = np.mean(coordinates[cluster_mask], axis=0)
        
        im = ax2.imshow(cluster_centers, cmap='viridis', aspect='auto')
        ax2.set_title(f'Cluster Mean Coordinates ({k_basis} heads)', fontsize=14, fontweight='bold')
        ax2.set_xlabel('Head Index')
        ax2.set_ylabel('Cluster')
        ax2.set_yticks(range(n_clusters))
        ax2.set_yticklabels([f'C{i}' for i in range(n_clusters)])
        ax2.set_xticks(range(k_basis))
        
        # head 수가 많으면 라벨 간격 조정
        if k_basis <= 8:
            ax2.set_xticklabels([f'H{i}' for i in range(k_basis)])
        else:
            # 8개 초과시 간격을 두고 표시
            step = max(1, k_basis // 8)
            ax2.set_xticks(range(0, k_basis, step))
            ax2.set_xticklabels([f'H{i}' for i in range(0, k_basis, step)])
        
        # 값 표시 (head 수가 많으면 생략)
        if k_basis <= 12:
            for i in range(n_clusters):
                for j in range(k_basis):
                    value = cluster_centers[i, j]
                    threshold = cluster_centers.max() * 0.5
                    text_color = "white" if value > threshold else "black"
        ...
        # 3. Source/Target Distribution by Cluster (Bar Chart)
        ax3 = axes[1, 0]
        cluster_stats = []
        ...
        for cluster_id in range(n_clusters):
            cluster_mask = cluster_assignments == cluster_id
            source_count = sum(1 for i, info in enumerate(source_target_info) 
                             if cluster_mask[i] and info == 'source')
            target_count = sum(1 for i, info in enumerate(source_target_info) 
                             if cluster_mask[i] and info == 'target')
            
            source_counts.append(source_count)
            target_counts.append(target_count)
            cluster_stats.append(f"C{cluster_id}: {source_count + target_count} (S:{source_count}, T:{target_count})")
        
        # 막대 그래프
        bars1 = ax3.bar(x_pos, source_counts, label='Source', color='cornflowerblue', alpha=0.8)
        bars2 = ax3.bar(x_pos, target_counts, bottom=source_counts, label='Target', 
                        color='lightcoral', alpha=0.8)
        
        ax3.set_title('Source/Target Distribution by Cluster', fontsize=14, fontweight='bold')
        ...
        for i, (s, t) in enumerate(zip(source_counts, target_counts)):
            ax3.text(i, s/2, str(s), ha='center', va='center', fontweight='bold')
            ax3.text(i, s + t/2, str(t), ha='center', va='center', fontweight='bold')
        
        # 4. Multi Source Clustering Summary (Text Box) - 가변 head 수 지원
        ax4 = axes[1, 1]
        ax4.axis('off')
        
        total_samples = len(coordinates)
        source_samples = sum(1 for info in source_target_info if info == 'source')
        target_samples = sum(1 for info in source_target_info if info == 'target')
        ...
        summary_text += f"Source Samples: {source_samples}\n"
        summary_text += f"Target Samples: {target_samples}\n"
        summary_text += f"Number of Heads: {k_basis}\n\n"
        
        # 클러스터별 상세 정보
        summary_text += "Cluster Details:\n"
        for i, (s, t) in enumerate(zip(source_counts, target_counts)):
            dominant_head = np.argmax(cluster_centers[i])
            dominant_value = cluster_centers[i, dominant_head]
            summary_text += f"C{i}: {source_counts[i] + target_counts[i]} samples (S:{source_counts[i]}, T:{target_counts[i]})\n"
            summary_text += f"  Dominant Head: H{dominant_head} ({dominant_value:.3f})\n\n"
        
        ax4.text(0.1, 0.9, summary_text, transform=ax4.transAxes, fontsize=10,
                verticalalignment='top', fontfamily='monospace',
                bbox=dict(boxstyle="round,pad=0.5", facecolor="lightblue", alpha=0.8))
        
        plt.suptitle('Multi Source Coordinate Clustering Analysis', fontsize=16, y=0.98)
        plt.tight_layout()
        
        # 저장
        clustering_viz_path = output_dir / 'clustering_results' / 'multi_source_clustering_analysis.png'
        clustering_viz_path.parent.mkdir(parents=True, exist_ok=True)
        fig.savefig(clustering_viz_path, dpi=300, bbox_inches='tight')
        plt.close(fig)
        
        logger.info(f"Multi-source clustering visualization saved: {clustering_viz_path} ({k_basis} heads)")
...
def main():
    parser = argparse.ArgumentParser(description='Attention Maps Inference (TabularFLM_S 지원)')
    ...
    parser.add_argument('--target_data', type=str, default='heart',
                        help='Target dataset name (default: heart)')
    ...
    # 1. Head Attention 히트맵 시각화 (새로 추가)
    logger.info("Generating head attention heatmaps...")
    head_viz_dir = Path(args.output_dir) / 'head_attention_heatmaps'
    # data_loader는 사용하지 않음 - source와 target을 개별적으로 처리
    inference.visualize_head_attention_heatmaps(None, head_viz_dir, args.max_samples)
    ...
    # 4. Multi-source 클러스터링 수행
    logger.info("Performing multi-source clustering...")
    clustering_results = inference.perform_multi_source_clustering(
        attention_data, 
    ...
    # 6. 클러스터별 attention maps 저장
    logger.info("Saving attention maps by cluster...")
    inference.save_attention_maps_by_cluster(
        attention_data,
        clustering_results, 
        clustering_results_dir,
        layer_idx=0  # basis layer 사용하므로 layer_idx는 의미없음
    )
