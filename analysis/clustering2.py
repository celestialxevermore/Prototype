"""
Attention Maps 기반 Inference 스크립트

저장된 attention maps를 로드하여 K-means 클러스터링을 수행합니다.

Usage:
    python inference_attention_map.py --attention_map_dir /path/to/attention_maps --n_clusters 8
"""
...
class AttentionMapInference:
    def __init__(self, attention_map_dir):
        ...
        logger.info(f"AttentionMapInference initialized for {self.num_layers} layers")
    ...
    def save_attention_maps_by_cluster(self, clustering_results, output_dir, layer_idx):
        ...
        for sample_idx, cluster_id in enumerate(clustering_results['cluster_assignments']):
            attention_map = self.attention_data[f'layer_{layer_idx}'][sample_idx]
            sample_id = self.attention_data['sample_ids'][sample_idx]
            label = self.attention_data['labels'][sample_idx]
            ...
            np.savez(cluster_dir / filename,
                    ...
                    feature_names=np.array(self.attention_data['feature_names']),
                    sample_id=sample_id,
                    label=label,
    ...
    def perform_clustering(self, layer_idx=2, n_clusters=5, output_dir=None):
        ...
        sample_ids = self.attention_data['sample_ids']
        feature_names = self.attention_data['feature_names']
    ...
    def _visualize_clustering_distribution(self, flattened_maps, cluster_assignments, labels, layer_idx, output_dir):
        """클러스터링 분포 시각화 (t-SNE)"""
        ...
        legend_added = {'clusters': set(), 'labels': set(), 'centroid': False}
    ...
    def _visualize_cluster_centroids(self, cluster_centers, feature_names, layer_idx, output_dir, n_clusters):
        ...
        logger.info(f"All {n_clusters} cluster centroids overview saved")
...
def main():
    parser = argparse.ArgumentParser(description='Attention Maps Based Inference')
    ...
    logger.info(f"\n{'='*80}")
    logger.info(f"🔍 ATTENTION MAP BASED INFERENCE")
    logger.info(f"{'='*80}")
    ...
    try:
        # Inference 실행
        inference = AttentionMapInference(args.attention_map_dir)
        ...
        # 모든 레이어에 대해 클러스터링
        for layer_idx in range(inference.num_layers):
