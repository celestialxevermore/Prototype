...
from utils.train_test import binary_train, binary_evaluate_, multi_train, multi_evaluate
...
def final_test_evaluate(model, test_loader, criterion, device, is_binary, threshold=None):
    ...
    """
    evaluate_func = binary_evaluate_ if is_binary else multi_evaluate
...
def pretrain_and_eval_sources(args, model, device, sources, patience=10):
    ...
    eval_fn  = binary_evaluate_ if is_bin else multi_evaluate
...
def main():
    ...
    # ===== Zero-shot 처리 =====
    if args.few_shot == 0:
        logger.info("[Zero-shot] Evaluating pretrained model directly on target test set...")
        
        evaluate_func = binary_evaluate_ if is_binary_t else multi_evaluate
        
        # Validation set에서 threshold 결정
        _, y_true_val, y_pred_val = evaluate_func(model_full, val_loader_t, crit_t, device)
        ...
        (test_loss_zero, test_auc_zero, test_precision_zero, test_recall_zero, 
         test_f1_zero, test_acc_zero, all_y_true_zero, all_y_pred_zero
            ...
            model_full, test_loader_t, crit_t, device, is_binary_t, threshold=best_threshold_zero
        )
